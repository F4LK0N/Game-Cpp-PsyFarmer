...
void Menu::addItem(string item){
	if(itens<10){
		this->pointerText.setFonte("fMenu"); //FONTE NÃO PODE SER SETADA NA INICIALIZAÇÃO PORQUE A FONTE AINDA NÃO FOI CARREGADA.
...
int Menu::run(){
	...
	//###############################################
	//UP
	if((gTeclado.pressionou[TECLA_CIMA] || gTeclado.pressionou[TECLA_W]) && pointer>0){
		pointer--; }
	//DOWN
	else if((gTeclado.pressionou[TECLA_BAIXO] || gTeclado.pressionou[TECLA_S]) && pointer+1<itens){
		pointer++; }
	//BACK
	else if(gTeclado.pressionou[TECLA_ESC] || gTeclado.pressionou[TECLA_VOLTAR] || gTeclado.pressionou[TECLA_A]){
		pointer=itens-1; }
	//###############################################
	...
	// SELECTED - KEYBOARD
	//###############################################
	if(gTeclado.pressionou[TECLA_ENTER] || gTeclado.pressionou[TECLA_ENTER2] || gTeclado.pressionou[TECLA_ESPACO] || gTeclado.pressionou[TECLA_D] || gTeclado.pressionou[TECLA_E]){
		soundClick.tocar();
		return pointer; }
	else{
		return -1; }
